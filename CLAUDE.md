...
LAN 메모/파일 전송용 윈도우 트레이 앱. `redmailbox/` 폴더에 격리.
경매 크롤러와 의존성·실행 분리.

### 위치
- 코드: `redmailbox/`
- 의존성: `redmailbox/requirements.txt` (pystray, Pillow, pyinstaller)
- 빌드: `redmailbox/build.bat` → `redmailbox/dist/RedMailbox.exe`
- 자세한 사용법: `redmailbox/README.md`

### 핵심 파일
| 파일 | 역할 |
|------|------|
| `redmailbox/redmailbox.py` | 진입점 (encoding_patch → server thread → tray) |
| `redmailbox/core/encoding_patch.py` | `socket.getfqdn` CP949 패치 (`open_map.py:14-24`와 동일 패턴) |
| `redmailbox/server/handler.py` | `/memo`, `/file`, `/ping` HTTP 핸들러 |
| `redmailbox/server/storage.py` | 파일 저장 + ` (1)` 자동 numbering + `.part` 원자적 rename |
| `redmailbox/client/sender.py` | urllib 송신, 진행률 콜백 |
| `redmailbox/tray/tray_app.py` | pystray.Icon (run_detached) + 메뉴 |
| `redmailbox/tray/blinker.py` | 수신 시 `icon.icon` idle/alert 토글 깜빡임 |
| `redmailbox/ui/*.py` | tkinter Toplevel 다이얼로그 (메모/파일/받은목록/설정) |

### 아키텍처
- 메인 스레드: tkinter `root.mainloop()` (hidden root + Toplevel)
- pystray: `icon.run_detached()` 별도 스레드
- HTTP 서버: `ThreadingHTTPServer` 데몬 스레드
- 메뉴 콜백 → `root.after(0, ...)`로 GUI 스레드 위임 (스레드 안전)

### HTTP 프로토콜
- `GET /ping` — 헬스체크 `{"ok":true}`
- `POST /memo` — JSON `{"text":"..."}`
- `POST /file` — body=raw bytes, header `X-Filename: <urllib.parse.quote(name)>` + `Content-Length`
  (multipart 회피, 64KB 청크 스트림, OOM 방지)

### 주의
- 한글 파일명은 X-Filename에 URL-encoded UTF-8 (송신/수신 모두)
- 받은 파일 같은 이름은 `(1)`, `(2)` 자동 numbering
- 설정/DB는 `%LOCALAPPDATA%/RedMailbox/`에 영속 (settings.json, history.db)
- 0.0.0.0 바인딩이지만 LAN 외부 접근은 Windows 방화벽이 차단 (첫 실행 시 허용 다이얼로그)
- 인증 없음 → 외부 노출 환경에서는 사용 금지

### 테스트
한 PC에서 두 프로세스: `set REDMAIL_PORT=8766 && set REDMAIL_SAVE=... && python redmailbox.py` (다른 환경변수)로 동시 실행 후 127.0.0.1 송수신.
자동: `cd redmailbox && python tests/test_roundtrip.py` (5개 시나리오)
