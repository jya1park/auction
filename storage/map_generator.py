...
def _geocode_items(data: List[Dict], addr_col: str) -> List[Dict]:
    ...
    for i, item in enumerate(data):
        ...
        if address in cache:
            ...
            cache[address] = coords  # None도 캐시 (반복 요청 방지)
            changed = True
...
<body>
  ...
  <script>
    ...
    function openCase(caseNum) {{
      if (!caseNum) return;
      var url = 'https://www.courtauction.go.kr/pgj/index.on?w2xPath=/pgj/ui/pgj100/PGJ158F00.xml';
      window.open(url, '_blank');
      if (navigator.clipboard) {{
        navigator.clipboard.writeText(caseNum)
          .then(function() {{ showToast(caseNum + ' 복사됨 — 법원경매 열림'); }})
          .catch(function() {{ showToast('법원경매 사이트를 열었습니다.'); }});
      }} else {{
        showToast('사건번호: ' + caseNum);
      }}
    }}
    ...
    function buildPopupContent(item) {{
      ...
      var btn = caseNum
        ? '<button class="iw-btn" onclick="openCase(\'' + caseNum.replace(/'/g, "\\'") + '\')">법원경매 바로가기</button>'
        : '';
    ...
    window.onload = function() {{
      ...
      ITEMS.forEach(function(item) {{
        ...
        bounds.extend(pos);
        mappedCount++;
        allEntries.push({{ marker: marker, type: type }});
      }});
      ...
      function applyFilter(type) {{
        ...
        closeIW();
        showToast(type === 'all' ? '전체 표시' : type + ' 필터 적용');
      }}

      function makeBtn(label, type, color) {{
        ...
        if (color) {{
          ...
          if (type === 'all') {{ btn.style.background = '#1F4E79'; btn.style.color = '#fff'; btn.style.borderColor = '#1F4E79'; }}
        }}
        btn.addEventListener('click', function() {{
          ...
          else {{ btn.style.background = '#1F4E79'; btn.style.color = '#fff'; }}
          applyFilter(type);
        }});
        return btn;
      }}

      wrap.appendChild(makeBtn('전체', 'all', null));
      types.forEach(function(type) {{
        wrap.appendChild(makeBtn(type, type, getColor(type)));
      }});
